...
class DateFilter extends Filter implements FilterContract
{
    ...
    public function handle(array $filters, Builder $query): Builder
    {
        ...
        if ($this->isBrand($filters) && isset($filters['date'])) {
            $query->whereHasMorph('rejectionable', MorphType::BRAND->value,
                fn(EloquentBuilder $builder) => $builder->where('brands.date', $this->getFilter($filters['date']))
            );
        }
    ...
    private function getFilter(Carbon $date): string
    {
